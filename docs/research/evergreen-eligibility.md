...
The published-post library and queue source picker offer a Top performing view. Users select a platform and one recorded count (views, impressions, reach, likes, comments, shares, saves, or clicks), and posts sort highest first before pagination. Search combines with these controls. Ranking uses only that platform's observation, excludes missing measurements, and retains measured zeroes. It never sums channels or actions, changes eligibility, or marks a post recommended. Ties resolve by publication date, then ID. The UI states the scope of up to 200 recent published posts for the brand and that raw counts are not adjusted for post age or audience size. Content review is still required for reuse.
