...
const FIGURE = "tabular-nums font-semibold text-slate-900 dark:text-slate-100";
...
export function LeaderboardTable({ rows }: { rows: AnalyticsPostRow[] }) {
  ...
  if (rows.length === 0) {
    return <div className="py-10 text-center text-[13px] text-slate-500 dark:text-slate-400">{t("empty")}</div>;
  }
        ...
        <table className="w-full min-w-[680px] border-collapse text-[12.5px]">
                ...
                return (
                  <th key={key} className="whitespace-nowrap px-2 py-2 text-end font-normal">
                    <button
                      ...
                      className={`-mx-1 -my-2.5 inline-flex cursor-pointer items-center gap-1 px-1 py-2.5 hover:text-slate-700 dark:hover:text-slate-200 ${EYEBROW}`}
          ...
          <tbody>
            {paginated.map((row) => {
              ...
              return [
                <tr key={row.id} className="border-b border-slate-100 transition-colors hover:bg-slate-50/60 dark:border-slate-800/60 dark:hover:bg-slate-800/30">
                  <td className="py-2.5 pe-3">
                    <div className="flex min-w-0 items-start gap-2.5">
                      <div className="flex shrink-0 items-center gap-1 pt-0.5">
                        {row.channels.map((ch) => <Channel key={ch} channel={ch} size={15} />)}
                      </div>
                      <span className="line-clamp-2 min-w-0 text-slate-900 dark:text-slate-100">{row.content || t("untitledPost")}</span>
                    </div>
                  </td>
                  ...
                  <td className="whitespace-nowrap px-2 text-end tabular-nums text-slate-500 dark:text-slate-400">{rateCell(row.erByReach, row.erByViews, t("byViews"))}</td>
                  <td className="whitespace-nowrap px-2 text-end text-[11px] tabular-nums text-slate-400 dark:text-slate-500">
                ...
                open ? (
                  <tr key={`${row.id}-history`} className="border-b border-slate-100 bg-slate-50/60 dark:border-slate-800/60 dark:bg-slate-800/20">
