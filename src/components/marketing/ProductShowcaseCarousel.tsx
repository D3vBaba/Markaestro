...
function PreviewShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="relative overflow-hidden rounded-xl border border-white/10 bg-[#f8fafc] shadow-2xl shadow-black/30">
      <div className="flex h-9 items-center gap-1.5 border-b border-border bg-card px-4">
      ...
      <div className="grid min-h-[330px] gap-4 p-4 sm:grid-cols-[1.15fr_0.85fr] sm:p-5">
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="flex items-center justify-between gap-3">
            <div className="flex items-center gap-2 text-xs font-semibold text-foreground">
              <Send className="h-3.5 w-3.5 text-mk-accent" />
              {rows[1]?.label}
            </div>
          ...
          <div className="mt-4 space-y-2 rounded-lg border border-border bg-muted/60 p-3">
            <span className="block h-2 w-full rounded-full bg-muted" />
            <span className="block h-2 w-[92%] rounded-full bg-muted" />
            <span className="block h-2 w-[68%] rounded-full bg-muted" />
          </div>
          ...
          <div className="mt-3 flex items-center justify-between gap-3">
            ...
            <span className="rounded-md bg-primary px-3 py-1.5 text-[10px] font-semibold text-primary-foreground">
        ...
        <div className="hidden flex-col gap-3 sm:flex">
          {channelProviders.slice(0, 3).map((provider, index) => (
            <div key={provider} className="flex-1 rounded-xl border border-border bg-card p-3">
              ...
              <div className="mt-3 flex gap-2">
                ...
                <div className="flex-1 space-y-1.5 pt-1">
                  ...
                  <span className="block h-1.5 w-[82%] rounded-full bg-muted" />
                  <span className="block h-1.5 w-[54%] rounded-full bg-muted" />
                </div>
              </div>
            </div>
          ))}
...
function CalendarPreview() {
  const posts = [
    ...
    { provider: "linkedin", column: 3, row: 1, tone: "bg-mk-accent-soft border-mk-accent/30" },
    { provider: "tiktok", column: 2, row: 2, tone: "bg-muted border-border" },
    { provider: "meta", column: 4, row: 2, tone: "bg-mk-accent-soft border-mk-accent/30" },
    { provider: "pinterest", column: 1, row: 3, tone: "bg-mk-neg-soft border-mk-neg/30" },
    { provider: "threads", column: 3, row: 3, tone: "bg-muted border-border" },
  ];
  ...
  return (
    <PreviewShell>
      <div className="min-h-[330px] p-4 sm:p-5">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 text-xs font-semibold text-foreground">
            <CalendarDays className="h-4 w-4 text-mk-accent" />
            <span className="h-2 w-28 rounded-full bg-primary" />
          </div>
          <div className="flex items-center gap-2">
            <span className="h-7 w-16 rounded-md bg-muted" />
            <span className="h-7 w-20 rounded-md bg-primary" />
          </div>
        </div>
        <div className="mt-4 grid grid-cols-4 overflow-hidden rounded-xl border border-border bg-card">
          {Array.from({ length: 16 }).map((_, index) => (
            <div key={index} className="min-h-[68px] border-b border-r border-mk-rule-soft p-1.5 last:border-r-0">
              <span className="text-[8px] font-medium text-mk-ink-40">{index + 8}:00</span>
            </div>
          ))}
          ...
          {posts.map((post) => (
            <div
              ...
              className={`relative z-10 col-span-1 m-1 -mt-[62px] flex h-[54px] items-center gap-2 rounded-lg border p-2 ${post.tone}`}
              style={{ gridColumn: post.column, gridRow: post.row + 1 }}
            >
              <ChannelGlyph provider={post.provider} size={22} className="rounded-md" />
              <div className="min-w-0 flex-1 space-y-1">
                <span className="block h-1.5 w-full rounded-full bg-mk-ink-20" />
                <span className="block h-1.5 w-2/3 rounded-full bg-mk-ink-20" />
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <PreviewShell>
      <div className="grid min-h-[330px] gap-4 p-4 sm:grid-cols-[1.08fr_0.92fr] sm:p-5">
        <div className="rounded-xl border border-border bg-card p-4">
          <div className="flex items-center gap-2 text-xs font-semibold text-foreground">
            <BarChart3 className="h-4 w-4 text-mk-accent" />
            {items[1]?.title}
          </div>
          <div className="mt-5 flex h-36 items-end gap-2 rounded-lg bg-muted/60 px-3 pb-3 pt-5">
          ...
          {items.slice(0, 3).map((item, index) => (
            <div
              ...
              className={`flex-1 rounded-xl border p-3.5 ${
                index === 1
                  ? "border-mk-accent/30 bg-mk-accent-soft"
            ...
            >
              <div className="flex items-center gap-2">
                ...
                <p className="text-[10px] font-semibold leading-tight text-foreground">{item.title}</p>
              </div>
...
export default function ProductShowcaseCarousel({
  ...
  return (
    <div
      className="relative overflow-hidden rounded-[28px] border border-border bg-[#0b1020] p-3 shadow-[0_28px_80px_-34px_rgba(37,99,235,0.58)] sm:p-5"
      onMouseEnter={() => setPaused(true)}
      onMouseLeave={() => setPaused(false)}
      onFocusCapture={() => setPaused(true)}
      onBlurCapture={() => setPaused(false)}
    >
      <div className="pointer-events-none absolute -left-24 -top-28 h-80 w-80 rounded-full bg-primary blur-3xl" />
      <div className="pointer-events-none absolute -bottom-28 right-8 h-80 w-80 rounded-full bg-fuchsia-600/20 blur-3xl" />

      <div className="relative flex gap-1 overflow-x-auto rounded-xl border border-white/10 bg-card/[0.06] p-1 scrollbar-hide">
          ...
          <button
            ...
            className={`min-w-max flex-1 rounded-xl px-4 py-2.5 text-left text-[11px] font-semibold transition sm:text-center sm:text-xs ${
              activeIndex === index
                ...
                : "text-white/60 hover:bg-card/[0.07] hover:text-white"
            }`}
        ...
        <AnimatePresence mode="wait" initial={false}>
          ...
          >
            <div className="max-w-md">
              ...
              <p className="mt-4 text-[13px] leading-relaxed text-mk-ink-40 sm:text-[14px]">
              ...
              <div className="mt-6 flex items-center gap-2">
                <button
                  ...
                  className="grid h-9 w-9 place-items-center rounded-full border border-white/15 text-white/75 transition hover:border-white/30 hover:bg-card hover:text-white"
                  ...
                  {slides.map((slide, index) => (
                    <span
                      ...
                      className={`h-1.5 rounded-full transition-all ${
                        activeIndex === index ? "w-6 bg-blue-400" : "w-1.5 bg-card"
                      }`}
                    />
                  ))}
                </div>
