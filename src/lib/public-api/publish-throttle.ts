...
export function getPublishDestinationKey(channel: SocialChannel, connection: PlatformConnection): string {
  ...
  if (channel === 'linkedin') {
    return `linkedin:${String(connection.metadata.linkedinDestinationAccountId || connection.metadata.linkedinProfileId || connection.productId || connection.workspaceId)}`;
  }
  if (channel === 'x') {
    return `x:${String(connection.metadata.xUserId || connection.metadata.username || connection.productId || connection.workspaceId)}`;
  }
