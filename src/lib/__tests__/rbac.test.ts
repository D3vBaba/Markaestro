...
describe('workspace permission matrix', () => {
  ...
  it('analyst is read-only', () => {
    ...
    expect(hasPermissionForRole('analyst', 'evergreen.read')).toBe(true);
    for (const permission of workspacePermissions) {
      if (permission === 'dashboard.read' || permission === 'intelligence.read' || permission === 'evergreen.read') continue;
