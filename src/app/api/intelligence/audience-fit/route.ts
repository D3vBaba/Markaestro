...
export async function POST(req: Request) {
  try {
    ...
    await consumeAiOperation({ workspaceId: ctx.workspaceId, uid: ctx.uid, monthlyLimit: resolveLimits(subscription).intelligenceAiOperationsPerMonth });
    const existing = await createAudienceFitJob({ workspaceId: ctx.workspaceId, uid: ctx.uid, request: input });
