...
import { consumeStrategistTurn } from '@/lib/intelligence/usage';
...
export async function POST(req: Request) {
  try {
    ...
    await consumeStrategistTurn({ workspaceId: ctx.workspaceId, uid: ctx.uid, monthlyLimit: limits.strategistTurnsPerMonth });
    const result = await askStrategist({ workspaceId: ctx.workspaceId, ...input });
