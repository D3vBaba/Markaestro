...
export async function POST(req: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (campaign.status !== 'active') {
      return apiOk({ ok: false, error: 'Only active campaigns can be paused' });
    }
    ...
    if (campaign.platform === 'meta') {
      ...
      result = await updateMetaCampaignStatus(accessToken, campaign.externalCampaignId, 'PAUSED');
      ...
      result = await updateTikTokCampaignStatus(accessToken, advertiserId, campaign.externalCampaignId, 'DISABLE');
    } else {
      return apiOk({ ok: false, error: `Unsupported platform: ${campaign.platform}` });
    }

    if (result.success) {
      await ref.update({ status: 'paused', updatedAt: new Date().toISOString() });
